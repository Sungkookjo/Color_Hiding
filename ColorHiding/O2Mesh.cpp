...
const D3DVERTEXELEMENT9 SHADOWVERT::Decl[4] =
{
...
//////////////////////////////////////////////////////////////////////////
// constructor
O2Mesh::O2Mesh(LPDIRECT3DDEVICE9* pDevice,Basefw_FS* FS)
:m_lpDevice(pDevice)
,m_pMesh(NULL)
...
HRESULT O2Mesh::Init( const TCHAR* Meshfilename , ResourceManager* pResource)
{	
	// 리소스 파일 메모리 주소를 읽어옴.
	char FileName[MAX_PATH];
	UINT _len = _tcslen(Meshfilename);
#ifdef UNICODE
	wcstombs(FileName,Meshfilename,_len);
	FileName[_len] = NULL;
#else
	strcpy_s(FileName,Meshfilename);
#endif
	...
	if(temp){
		if(FAILED(hr=D3DXLoadMeshFromXInMemory(temp,
		...
		{
			MessageBox(NULL, _T("FAILED : D3DXLoadMeshFromX"), Meshfilename, MB_OK);
			return E_FAIL;
		}
	}else{
		MessageBox(NULL, _T("FAILED : ReadFile"), Meshfilename, MB_OK);
		return E_FAIL;
	}
	...
	for( DWORD i=0; i<m_dwNumMaterials; i++ )
	{
		...
		{
			TCHAR filename[125];
			int len = strlen(d3dxMaterials[i].pTextureFilename);
#ifdef UNICODE
			mbstowcs(filename,d3dxMaterials[i].pTextureFilename,len);
			filename[len] = '\0';
			m_pMeshTextures[i] = pResource->GetTexture(filename);
#else
			m_pMeshTextures[i] = pResource->GetTexture(d3dxMaterials[i].pTextureFilename);
#endif
			// Create the texture
			if( NULL == m_pMeshTextures[i])
			{
				_DEBUG_LOG(_T("Failed-LoadTexutre : [ %s ] \n"),filename);
			}
		}
	}
	...
	{
		_DEBUG_LOG(_T("Failed - OptimizeInplace() [%s]\n"),Meshfilename);
	...
	m_pMesh->GetIndexBuffer( &m_lpIndexBuffer );
	m_pMesh->GetVertexBuffer( &m_lpVertexBuffer );
	m_uNumFaces = m_pMesh->GetNumFaces();
	m_uNumIndeces = m_uNumFaces * 3;
	m_uNumVertices = m_pMesh->GetNumVertices();

	m_pColl = new O2CollisionInfo;
	ComputeBounding(&m_pMesh,m_pColl);
...
HRESULT O2Mesh::GenerateShadowMesh()
{
	HRESULT hr = S_OK;
