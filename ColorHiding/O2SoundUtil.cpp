...
IDirectSoundBuffer* O2SoundUtil::DSLoadSoundBuffer(IDirectSound *pDS, LPCTSTR lpName)
{
...
IDirectSoundBuffer* O2SoundUtil::DSLoad3DSoundBuffer(IDirectSound *pDS, LPCTSTR lpName)
{
	IDirectSoundBuffer *pDSB = NULL;
	DSBUFFERDESC dsBD = {0};
	BYTE *pbWaveData;
...
BOOL O2SoundUtil::DSReloadSoundBuffer(IDirectSoundBuffer *pDSB, LPCTSTR lpName)
{
	BOOL result=FALSE;
	BYTE *pbWaveData;
	DWORD cbWaveSize;
...
//
///////////////////////////////////////////////////////////////////////////////
BOOL O2SoundUtil::DSGetWaveResource(HMODULE hModule, LPCTSTR lpName,
					   WAVEFORMATEX **ppWaveHeader, BYTE **ppbWaveData, DWORD *pcbWaveSize)
{
	...
	if (((hResInfo = FindResource(hModule, lpName, _T("WAV"))) != NULL) &&
...
//
///////////////////////////////////////////////////////////////////////////////
BOOL O2SoundUtil::DSGetWaveFile(HMODULE hModule, LPCTSTR lpName,
				   WAVEFORMATEX **ppWaveHeader, BYTE **ppbWaveData, DWORD *pcbWaveSize,
				   void **ppvBase)
{
	void *pvRes;
...
SNDOBJ* O2SoundUtil::SndObjCreate(IDirectSound *pDS, LPCTSTR lpName, int iConcurrent,BOOL _3Dtype)
{
	SNDOBJ *pSO = NULL;
	...
	if( fFound )
	{
			...
			(iConcurrent-1) * sizeof(IDirectSoundBuffer *))) != NULL)
		{
			...
			if(_3Dtype)
			{
				pSO->Buffers[0] = DSLoad3DSoundBuffer(pDS, lpName);
				...
				for (i=1; i<pSO->iAlloc; i++)
				{
					...
					{
						pSO->Buffers[i] = DSLoad3DSoundBuffer(pDS, lpName);
						if (!pSO->Buffers[i]) {
							SndObjDestroy(pSO);
							pSO = NULL;
							break;
...
void O2SoundUtil::SndObjDestroy(SNDOBJ *pSO)
{
	if (pSO)
	{
		int i;
...
IDirectSoundBuffer* O2SoundUtil::SndObjGetFreeBuffer(SNDOBJ *pSO)
{
...
BOOL O2SoundUtil::SndObjPlay(SNDOBJ *pSO, DWORD dwPlayFlags)
{
	BOOL result = FALSE;
...
BOOL O2SoundUtil::SndObjIsPlay(SNDOBJ *pSO, DWORD dwPlayFlags)
{
	BOOL result = FALSE;
	...
	if ((pDSB = pSO->Buffers[pSO->iCurrent]) /*&& (dwPlayFlags == NULL)*/)
	{
		HRESULT hres;
		DWORD dwStatus;
...
BOOL O2SoundUtil::SndObjStop(SNDOBJ *pSO)
{
	int i;
...
BOOL O2SoundUtil::SndObjPause(SNDOBJ *pSO)
{
	int i;
...
BOOL O2SoundUtil::DSFillSoundBuffer(IDirectSoundBuffer *pDSB, BYTE *pbWaveData, DWORD cbWaveSize)
{
...
BOOL O2SoundUtil::DSParseWaveResource(void *pvRes, WAVEFORMATEX **ppWaveHeader, BYTE **ppbWaveData,DWORD *pcbWaveSize)
{
	DWORD *pdw;
	DWORD *pdwEnd;
	DWORD dwRiff;
